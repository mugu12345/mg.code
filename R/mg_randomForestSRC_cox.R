mg_randomForestSRC_cox=function(dat,time,event){
  ...
  dat1=dat[,match(sel.genes,colnames(dat))]
  cx.mods=list()
  cindx=c()

  cx.modl=mg_createCoxModel_auto_step(dat1,time = dat$days,event = dat$status)
  if(!is.null(cx.modl)){
    c1=getC_index(riskscore = cx.modl$Score,os = dat$days,status = dat$status)
    cindx=c(cindx,c1$c.index)
  }else{
    cindx=c(cindx,NA)
  }
  cx.mods=c(cx.mods,list(cx.modl))

  for(i in 1:3){
    sel.genes=all.vars[[i]]
  ...
  cx.modl=cx.mods[[which.max(cindx)]]
  lst.genes=cx.modl$Genes
  ...
  return(list(plot=gall.all,Genes=lst.genes,Coef=cx.modl$Coef,Model=cx.modl))
}
